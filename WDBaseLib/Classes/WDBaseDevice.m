...
@implementation WDBaseDevice (Security)
...
@implementation WDBaseDevice (DeviceIdentifier)
#pragma mark - Related to device identifier

// IDFA
+ (NSString *)getDeviceIDFA {
    __block NSString *derivedIdFA = [NSString string];
    if (@available(iOS 14, *)) {
        // iOS14及以上版本需要先请求权限
        [ATTrackingManager requestTrackingAuthorizationWithCompletionHandler:^(ATTrackingManagerAuthorizationStatus status) {
            ...
            derivedIdFA = [[ASIdentifierManager sharedManager].advertisingIdentifier UUIDString];
        }];
    } else {
        // iOS14以下版本依然使用老方法
...
@implementation WDBaseDevice (ScreenResolution)
...
@implementation WDBaseDevice (Battery)
...
+ (NSString *)getDeviceBatteryLevel{
    ...
    int percentBattery = (int)([UIDevice currentDevice].batteryLevel * 100);
    return  [NSString stringWithFormat:@"%d",percentBattery];
}
...
@implementation WDBaseDevice (Carrier)
...
+ (NSString *)getCountryCode {
    ...
    //将返回实际的国家代码NSLog(@"ISO 国家代码为:%@", carrier.isoCountryCode);
    NSLog(@"国家代码为:%@",carrier.mobileCountryCode);
...
+ (NSString *)getNetworkCode {
    ...
    CTCarrier *carrier = network_Info.subscriberCellularProvider;
    //将返回实际的国家代码NSLog(@"ISO 国家代码为:%@", carrier.isoCountryCode);
    NSLog(@"网络代码为:%@",carrier.mobileNetworkCode);
...
@implementation WDBaseDevice (WiFi)
...
@implementation WDBaseDevice (Disk)
...
@implementation WDBaseDevice (Memory)

// 设备内存
+ (NSString *)getDeviceTotalMemory {
    NSLog(@"memory: %lld",[NSProcessInfo processInfo].physicalMemory);
...
+ (NSString *)getActiveMemory {
    ...
    if (kern != KERN_SUCCESS) return @"-1";
    long long activateSize = vm_stat.active_count * page_size;
    NSLog(@"Active memory size: %@",[[NSNumber numberWithLongLong:activateSize] stringValue]);
    return [[NSNumber numberWithLongLong:activateSize] stringValue];
}
...
+ (NSString *)getInActiveMemory {
    ...
    kern = host_statistics(host_port, HOST_VM_INFO, (host_info_t)&vm_stat, &host_size);
    if (kern != KERN_SUCCESS) return @"-1";
    long long inactivateSize = vm_stat.inactive_count * page_size;
    NSLog(@"Inactive memory size: %@",[[NSNumber numberWithLongLong:inactivateSize] stringValue]);
    return [[NSNumber numberWithLongLong:inactivateSize] stringValue];
}
...
+ (NSString *)getWiredMemory {
    ...
    kern = host_statistics(host_port, HOST_VM_INFO, (host_info_t)&vm_stat, &host_size);
    if (kern != KERN_SUCCESS) return @"-1";
    long long wiredMemorySize = vm_stat.wire_count * page_size;
    NSLog(@"wired memory size: %@",[[NSNumber numberWithLongLong:wiredMemorySize] stringValue]);
    return [[NSNumber numberWithLongLong:wiredMemorySize] stringValue];
}
...
+ (NSString *)getUsedMemory {
    ...
    kern = host_statistics(host_port, HOST_VM_INFO, (host_info_t)&vm_stat, &host_size);
    if (kern != KERN_SUCCESS) return @"-1";
    long long usedMemorySize = page_size * (vm_stat.active_count + vm_stat.inactive_count + vm_stat.wire_count);
    NSLog(@"wired memory size: %@",[[NSNumber numberWithLongLong:usedMemorySize] stringValue]);
    return [[NSNumber numberWithLongLong:usedMemorySize] stringValue];
}
...
+ (NSString *)getPurgableMemory {
    ...
    kern = host_statistics(host_port, HOST_VM_INFO, (host_info_t)&vm_stat, &host_size);
    if (kern != KERN_SUCCESS) return @"-1";
    long long purgableMemorySize =vm_stat.purgeable_count * page_size;
    NSLog(@"wired memory size: %@",[[NSNumber numberWithLongLong:purgableMemorySize] stringValue]);
    return [[NSNumber numberWithLongLong:purgableMemorySize] stringValue];
}
